...
BOOL Search_Module( void* Path )
{
    ...
    CFURLRef moduleUrl = CFURLCreateCopyAppendingPathComponent( kCFAllocatorDefault, parentFolderUrl, CFSTR("Type0016 Module.bundle"), true );
...
#elif defined(__APPLE__)
	// Create CFBundle object from CFURLRef
		...
		case kNkMAIDCapability_ExposureMode:
			switch( ulValue ){
				...
				case kNkMAIDExposureMode_Scene:
					strcpy( psString, "SCENE Other scenes" );
					break;
...
char*	GetUnsignedString( ULONG ulCapID, ULONG ulValue, char *psString )
{
	...
	case kNkMAIDCapability_LiveViewProhibit:
		...
		{
			strcat( psString, "Exposure Mode is non-P,S,A,M, " );
		}
	...
	case kNkMAIDCapability_MovieScreenSize:
		...
		{
			if (g_ulVideoMode == kNkMAIDVideoMode_NTSC)
			{
				sprintf(buff, "%d : 1920x1080 60p\n", kNkMAIDMovieScreenSize3_FullHD_1);
				strcpy(psString, buff);
				sprintf(buff, "%d : 1920x1080 30p\n", kNkMAIDMovieScreenSize2_FullHD_2);
				strcat(psString, buff);
				sprintf(buff, "%d : 1920x1080 24p\n", kNkMAIDMovieScreenSize3_FullHD_3);
				strcat(psString, buff);
				sprintf(buff, "%d : 1280x 720 60p\n", kNkMAIDMovieScreenSize3_720p);
				strcat(psString, buff);
				sprintf(buff, "%d :  624x 424 30p\n", kNkMAIDMovieScreenSize3_VGA);
				strcat(psString, buff);
			}
			else if (g_ulVideoMode == kNkMAIDVideoMode_PAL)
			{
				...
				sprintf(buff, "%d : 1920x1080 24p\n", kNkMAIDMovieScreenSize3_FullHD_3);
				strcat(psString, buff);
				sprintf(buff, "%d : 1280x 720 50p\n", kNkMAIDMovieScreenSize3_720p);
				strcat(psString, buff);
				sprintf(buff, "%d :  624x 424 25p\n", kNkMAIDMovieScreenSize3_VGA);
				strcat(psString, buff);
			}
		}
		else if (g_ulCameraType == kNkMAIDCameraType_D5600)
		{
			sprintf(buff, "%d : 1920x1080 60p\n", kNkMAIDMovieScreenSize3_FullHD_1);
			strcpy(psString, buff);
			sprintf(buff, "%d : 1920x1080 50p\n", kNkMAIDMovieScreenSize2_FullHD_2);
			...
			sprintf(buff, "%d : 1920x1080 30p\n", kNkMAIDMovieScreenSize3_FullHD_3);
			strcat(psString, buff);
			sprintf(buff, "%d : 1920x1080 25p\n", kNkMAIDMovieScreenSize3_FullHD_4);
			strcat(psString, buff);
			sprintf(buff, "%d : 1920x1080 24p\n", kNkMAIDMovieScreenSize3_FullHD_5);
			strcat(psString, buff);
			sprintf(buff, "%d : 1280x 720 60p\n", kNkMAIDMovieScreenSize3_720p);
			strcat(psString, buff);
			sprintf(buff, "%d : 1280x 720 50p\n", kNkMAIDMovieScreenSize3_720p_2);
			strcat(psString, buff);
...
BOOL GetLiveViewImageCapability( LPRefObj pRefSrc )
{
	...
	if (g_ulCameraType == kNkMAIDCameraType_D5500 || g_ulCameraType == kNkMAIDCameraType_D5600)
	{
		ulHeaderSize = 384;
	}
